
#include<iostream>
#include<stdio.h>
#include<stdlib.h>
#include<fstream>
#include<string.h>
#include<conio.h>
#include <iomanip>
#include <sstream> // Include the <sstream> header

// ...
using namespace std;
class Lib
{
   public:
       char bookname[100],auname[50],sc[20],sc1[50];
       int q,B,p;
       Lib()
       {
           strcpy(bookname,"NO Book Name");
           strcpy(auname,"No Author Name");
           strcpy(sc,"No Book ID");
           strcpy(sc1,"No Book ID");
           q=0;
           B=0;
           p=0;
       }
          void get();
          void student();
          void pass();
          void librarian();
          void password();
          void getdata();
          void show(int);
          void booklist(int);
          void modify();
          void see(int);
          int branch(int);
          void issue();
          void ProjectName();
          void der(char[],int,int);
          void fine(int,int,int,int,int,int);
};
void Lib::getdata()
{
                    int i;
                    fflush(stdin);
                    cout<<"\n\t\t\t\t\t\tEnter The Details :-\n";
                    cout<<"\n\t\t\t\t\t\tEnter Book's Name : ";
                    cin.getline(bookname,100);
                    for(i=0;bookname[i]!='\0';i++)
                    {
                    if(bookname[i]>='a'&&bookname[i]<='z')
                       bookname[i]-=32;
                    }
                    cout<<"\n\t\t\t\t\t\tEnter Author's Name : ";
                    cin.getline(auname,50);
                    cout<<"\n\t\t\t\t\t\tEnter Publication name : ";
                    cin.getline(sc1,50);
                    cout<<"\n\t\t\t\t\t\tEnter Book's ID : ";
                    cin.getline(sc,20);
                    cout<<"\n\t\t\t\t\t\tEnter Book's Price : ";
                    cin>>p;
                    cout<<"\n\t\t\t\t\t\tEnter Book's Quantity : ";
                    cin>>q;
}

void Lib::show(int i)
{
    cout << "\t\t\t\t-------------------------------------------------------------" << endl;
    cout << "\t\t\t\t| Book Name           : " << setw(35)<< left << bookname << " |" << endl;
    cout << "\t\t\t\t| Book's Author Name  : " << setw(35)<< left << auname << " |" << endl;
    cout << "\t\t\t\t| Book's ID           : " << setw(35)<< left << sc << " |" << endl;
    cout << "\t\t\t\t| Book's Publication  : " << setw(35)<< left << sc1 << " |" << endl;
    if (i == 2)
    {
        cout << "\t\t\t\t| Book's Price        : " << setw(35)<< left << p << " |" << endl;
        cout << "\t\t\t\t| Book's Quantity     : " << setw(35)<< left << q << " |" << endl;
    }
    cout << "\t\t\t\t-------------------------------------------------------------" << endl;
}
void Lib::booklist(int i)
{
    int b, r = 0;
    system("cls");
    b = branch(i);
    system("cls");
    ifstream intf("Booksdata.txt", ios::binary);
    if (!intf)
        cout << "\n\t\tFile Not Found.";
    else
    {
        cout << "\n\t\t\t\t                   Book List                \n\n";

        // Print table headers
        cout << "------------------------------------------------------------------------------------------------------------------\n";
        cout << "| S.No.  |               Book Name              |            Author Name         |   Book ID |  Price  | Quantity|\n";
        cout << "------------------------------------------------------------------------------------------------------------------\n";

        while (intf.read((char *)this, sizeof(*this)))
        {
            if (b == B)
            {
                if (q == 0 && i == 1)
                {
                    // Skip displaying books with quantity 0 for students
                }
                else
                {
                    r++;
                    // Print table row with proper alignment
                    cout << "| " << setw(6) << left << r << " | " << setw(36) << left << bookname << " | " << setw(30) << left << auname << " | " << setw(10) << left << sc << "| " << setw(7) << p << " | " << setw(8) << q << "|\n";
                    cout << "-----------------------------------------------------------------------------------------------------------------\n";
                }
            }
        }
    }
    intf.close();

    cout << "\n\t\t\t\t\t\tPress Any Key To Continue.....";
    getch();
    system("cls");
    if (i == 1)
        student();
    else
        librarian();
}


  void Lib::modify()
  {
    char ch,st1[100];
    int i=0,b,cont=0;
    system("cls");
    cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
    cout<<"\t\t\t|                      >>Please Choose One Option :-                    |\n";
    cout<<"\t\t\t|1. Modification In Current Books                                       |\n";
    cout<<"\t\t\t|2. Add New Book                                                        |\n";
    cout<<"\t\t\t|3. Delete A Book                                                       |\n";
    cout<<"\t\t\t|4. Go back                                                             |\n";
    cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
    cout<<"\n\n\t\t\t\t\t\t=>Enter Your Choice : ";
    cin>>i;
    if(i==1)
    {
                    system("cls");
                    b=branch(2);
                    ifstream intf1("Booksdata.txt",ios::binary);
                    if(!intf1)
                    {
                        cout<<"\n\t\t\t\t\t\tFile Not Found\n";
                        cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
                        getch();
                        system("cls");
                        librarian();
                    }
                        intf1.close();
                        system("cls");
                        cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
                        cout<<"\t\t\t|                      LIBRARY MANAGEMENT SYSTEM                        |\n";
                        cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
                        cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
                        cout<<"\t\t\t|                      Please Choose One Option :-                      |\n";
                        cout<<"\t\t\t|1. Search By Name                                                      |\n";
                        cout<<"\t\t\t|2. Search By Book's ID                                                 |\n";
                        cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
                        cout<<"\n\t\t\t\t\t\t=>Enter Your Choice : ";
                        cin>>i;
                        fflush(stdin);
                        if(i==1)
                            {
                                system("cls");
                                cout<<"\n\t\t\t\t\t\tEnter Book Name : ";
                                cin.getline(st1,100);
                                system("cls");
                                fstream intf("Booksdata.txt",ios::in|ios::out|ios::ate|ios::binary);
                                intf.seekg(0);
                                intf.read((char*)this,sizeof(*this));
                                while(!intf.eof())
                                    {
                                        for(i=0;b==B&&bookname[i]!='\0'&&st1[i]!='\0'&&(st1[i]==bookname[i]||st1[i]==bookname[i]+32);i++);
                                        if(bookname[i]=='\0'&&st1[i]=='\0')
                                            {
                                                cont++;
                                                getdata();
                                                intf.seekp(intf.tellp()-sizeof(*this));
                                                intf.write((char*)this,sizeof(*this));
                                                break;
                                            }
                                                     intf.read((char*)this,sizeof(*this));
                                    }
                                    intf.close();
                            }
                        else if(i==2)
                        {
                                cout<<"\n\t\t\t\t\t\tEnter Book's ID : ";
                                cin.getline(st1,100);
                                system("cls");
                                fstream intf("Booksdata.txt",ios::in|ios::out|ios::ate|ios::binary);
                                intf.seekg(0);
                                intf.read((char*)this,sizeof(*this));
                                while(!intf.eof())
                                    {
                                        for(i=0;b==B&&sc[i]!='\0'&&st1[i]!='\0'&&st1[i]==sc[i];i++);
                                        if(sc[i]=='\0'&&st1[i]=='\0')
                                            {
                                                cont++;
                                                getdata();
                                                intf.seekp(intf.tellp()-sizeof(*this));
                                                intf.write((char*)this,sizeof(*this));
                                                break;
                                            }
                                        intf.read((char*)this,sizeof(*this));
                                    }

                                intf.close();
                        }
                        else
                        {
                            cout<<"\n\t\t\t\t\t\tIncorrect Input.....:(\n";
                            cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
                            getch();
                            system("cls");
                            modify();
                        }
                        if(cont==0)
                        {
                            cout<<"\n\t\t\t\t\t\tBook Not Found.\n";
                            cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
                            getch();
                            system("cls");
                            modify();
                        }
                        else
                            cout<<"\n\t\t\t\t\t\tUpdate Successful.\n";


    }
    else if(i==2)
    {
                    system("cls");
                    B=branch(2);
                    system("cls");
                    getdata();
                    ofstream outf("Booksdata.txt",ios::app|ios::binary);
                    outf.write((char*)this,sizeof(*this));
                    outf.close();
                    cout<<"\n\t\t\t\t\t\tBook Added Successfully.\n";
    }
    else if(i==3)
    {
                    system("cls");
                    b=branch(2);
                    ifstream intf1("Booksdata.txt",ios::binary);
                    if(!intf1)
                    {
                        cout<<"\n\t\t\t\t\t\tFile Not Found\n";
                        cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
                        getch();
                        intf1.close();
                        system("cls");
                        librarian();
                    }
                        intf1.close();
                        system("cls");
                        cout<<"\n\t\t\t\t\t\tPlease Choose One Option For Deletion:-\n";
                        cout<<"\n\t\t\t\t\t\t1. By Book Name\n\n\t\t\t\t\t\t2. By Book's ID\n";
                        cout<<"\n\t\t\t\t\t\tEnter Your Choice : ";
                        cin>>i;
                        fflush(stdin);
                        if(i==1)
                            {
                                system("cls");
                                cout<<"\n\t\t\t\t\t\tEnter Book Name : ";
                                cin.getline(st1,100);
                                ofstream outf("temp.txt",ios::app|ios::binary);
                                ifstream intf("Booksdata.txt",ios::binary);
                                intf.read((char*)this,sizeof(*this));
                                while(!intf.eof())
                                    {
                                        for(i=0;b==B&&bookname[i]!='\0'&&st1[i]!='\0'&&(st1[i]==bookname[i]||st1[i]==bookname[i]+32);i++);
                                        if(bookname[i]=='\0'&&st1[i]=='\0')
                                            {
                                                cont++;
                                                intf.read((char*)this,sizeof(*this));

                                            }
                                        else
                                        {
                                        outf.write((char*)this,sizeof(*this));
                                        intf.read((char*)this,sizeof(*this));
                                        }
                                    }

                    intf.close();
                    outf.close();
                    remove("Booksdata.txt");
                    rename("temp.txt","Booksdata.txt");
                            }
                        else if(i==2)
                        {
                                cout<<"\n\t\t\t\t\t\tEnter Book's ID : ";
                                cin.getline(st1,100);
                                ofstream outf("temp.txt",ios::app|ios::binary);
                                ifstream intf("Booksdata.txt",ios::binary);
                                intf.read((char*)this,sizeof(*this));
                                while(!intf.eof())
                                    {
                                        for(i=0;b==B&&sc[i]!='\0'&&st1[i]!='\0'&&st1[i]==sc[i];i++);
                                        if(sc[i]=='\0'&&st1[i]=='\0')
                                            {
                                                cont++;
                                                intf.read((char*)this,sizeof(*this));
                                            }
                                        else
                                        {
                                        outf.write((char*)this,sizeof(*this));
                                        intf.read((char*)this,sizeof(*this));
                                        }
                                    }
                    outf.close();
                    intf.close();
                    remove("Booksdata.txt");
                    rename("temp.txt","Booksdata.txt");
                        }
                        else
                        {
                            cout<<"\n\t\t\t\t\t\tIncorrect Input.....:(\n";
                            cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
                            getch();
                            system("cls");
                            modify();
                        }
                        if(cont==0)
                        {
                            cout<<"\n\t\t\t\t\t\tBook Not Found.\n";
                            cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
                            getch();
                            system("cls");
                            modify();
                        }
                        else
                            cout<<"\n\t\t\t\t\t\tDeletion Successful.\n";

    }
    else if(i==4)
    {
    system("cls");
    librarian();
    }
    else
    {
    cout<<"\n\t\t\t\t\t\tWrong Input.\n";
    cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
    getch();
    system("cls");
    modify();
    }
    cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
    getch();
    system("cls");
    librarian();

  }
  int Lib::branch(int x)
  {
      int i;
      cout<<"\t\t\t+------------------------------------------------------------------------+\n";
      cout<<"\t\t\t|                        LIBRARY MANAGEMENT SYSTEM                       |\n";
      cout<<"\t\t\t+------------------------------------------------------------------------+\n";
      cout<<"\t\t\t+------------------------------------------------------------------------+\n";
      cout<<"\t\t\t|                        >>Please Choose One Branch :-                   |\n";
      cout<<"\t\t\t|1. 1st Year 1st Semester                                                |\n";
      cout<<"\t\t\t|2. 1st Year 2nd Semester                                                |\n";
      cout<<"\t\t\t|3. 2nd Year 1st Semester                                                |\n";
      cout<<"\t\t\t|4. 2nd Year 2nd Semester                                                |\n";
      cout<<"\t\t\t|5. 3rd Year 1st Semester                                                |\n";
      cout<<"\t\t\t|6. 3rd Year 2nd Semester                                                |\n";
      cout<<"\t\t\t|7. Go To Menu                                                           |\n";
      cout<<"\t\t\t+------------------------------------------------------------------------+\n";
      cout<<"\n\t\t\t\t\t\t=>Enter Your Choice : ";
      cin>>i;
      switch(i)
      {
          case 1: return 1;
                  break;
          case 2: return 2;
                  break;
          case 3: return 3;
                  break;
          case 4: return 4;
                  break;
          case 5: return 5;
                  break;
          case 6: return 6;
                  break;
          case 7: system("cls");
                  if(x==1)
                  student();
                  else
                    librarian();
          default : cout<<"\n\t\t\t\t\t\tPlease Enter Correct Option :(";
                    getch();
                    system("cls");
                    branch(x);
        }
  }
  void Lib::see(int x)
  {
      int i,b,cont=0;
      char ch[100];
      system("cls");
      b=branch(x);
      ifstream intf("Booksdata.txt",ios::binary);
        if(!intf)
        {
            cout<<"\n\t\t\t\t\t\tFile Not Found.\n";
            cout<<"\n\t\t\t\t\t  Press Any Key To Continue.....";
            getch();
            system("cls");
            if(x==1)
            student();
            else
            librarian();
        }

      system("cls");
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\t\t\t|                      LIBRARY MANAGEMENT SYSTEM                        |\n";
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\t\t\t|                      Please Choose One Option :-                      |\n";
      cout<<"\t\t\t|1. Search By Name                                                      |\n";
      cout<<"\t\t\t|2. Search By Book's ID                                                 |\n";
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\n\t\t\t\t\t\t=>Enter Your Choice : ";
      cin>>i;
      fflush(stdin);
      intf.read((char*)this,sizeof(*this));
      if(i==1)
      {
          cout<<"\n\t\t\t\t\t\tEnter Book's Name : ";
          cin.getline(ch,100);
          system("cls");
          while(!intf.eof())
          {
            for(i=0;b==B&&q!=0&&bookname[i]!='\0'&&ch[i]!='\0'&&(ch[i]==bookname[i]||ch[i]==bookname[i]+32);i++);
            if(bookname[i]=='\0'&&ch[i]=='\0')
                {
                        cout<<"\n\t\t\t\t\t\t\tBook Found :-\n";
                        show(x);
                        cont++;
                        break;
                }
             intf.read((char*)this,sizeof(*this));
          }
      }
          else if(i==2)
          {
          cout<<"\n\t\t\t\t\t\tEnter Book's ID : ";
          cin.getline(ch,100);
          system("cls");
          while(!intf.eof())
          {
              for(i=0;b==B&&q!=0&&sc[i]!='\0'&&ch[i]!='\0'&&ch[i]==sc[i];i++);
              if(sc[i]=='\0'&&ch[i]=='\0')
                {
                            cout<<"\n\t\t\t\t\t\t\tBook Found :-\n";
                            show(x);
                            cont++;
                            break;
                }
               intf.read((char*)this,sizeof(*this));
          }

          }
          else
          {
             cont++;
             cout<<"\n\t\t\t\t\t\tPlease Enter Correct Option :(";
             getch();
             system("cls");
             see(x);
          }
          intf.close();
          if(cont==0)
              cout<<"\n\t\t\t\t\t\tThis Book Is Not Available :( \n";

    cout<<"\n\t\t\t\t\t\tPress Any Key To Continue.....";
    getch();
    system("cls");
    if(x==1)
    student();
    else
    librarian();


  }
void Lib::issue()
{
    char st[50],st1[20];
    int b,i,j,d,m,y,dd,mm,yy,cont=0;
    system("cls");
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\t\t\t|                      LIBRARY MANAGEMENT SYSTEM                        |\n";
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\t\t\t|                   ->Please Choose One Option :-                       |\n";
      cout<<"\t\t\t|1. Issue Book                                                          |\n";
      cout<<"\t\t\t|2. View Issued Book                                                    |\n";
      cout<<"\t\t\t|3. Search Student Who Issued Books                                     |\n";
      cout<<"\t\t\t|4. Reissue Book                                                        |\n";
      cout<<"\t\t\t|5. Return Book                                                         |\n";
      cout<<"\t\t\t|6. Go back to menu                                                     |\n";
      cout<<"\t\t\t+-----------------------------------------------------------------------+\n";
      cout<<"\t\t\t\t\t\t=>Enter Your Choice : ";
      cin>>i;
    fflush(stdin);
    if(i==1)
    {
    system("cls");
    b=branch(2);
    system("cls");
    fflush(stdin);
    cout<<"\n\t\t\t\t\t\t->Please Enter Details :-\n";
    cout<<"\n\t\t\t\t\t\tEnter Book Name : ";
    cin.getline(bookname,100);
    cout<<"\n\t\t\t\t\t\tEnter Book's ID : ";
    cin.getline(sc,20);
    //strcpy(st,sc);
    der(sc,b,1);
    cout<<"\n\t\t\t\t\t\tEnter Student Name : ";
    cin.getline(auname,100);
    cout<<"\n\t\t\t\t\t\tEnter Student's ID : ";
    cin.getline(sc1,20);
    cout<<"\n\t\t\t\t\t\tEnter date : ";
    cin>>q>>B>>p;
    ofstream outf("student.txt",ios::binary|ios::app);
    outf.write((char*)this,sizeof(*this));
    outf.close();
    cout<<"\n\n\t\t\t\t\t\tIssue Successfully.\n";
    }

else if (i == 2)
{
    ifstream intf("student.txt", ios::binary);
    system("cls");
    cout << "\n\t\t\t\t\t\t->The Details are :-\n";

    // Print table headers
    cout << "+-------------------------------------------------------------------------------------------------------+\n";
    cout << "| S.No.   |     Student Name     |   Student's ID   |      Book Name      |    Book's ID   |      Date  |\n";
    cout << "+-------------------------------------------------------------------------------------------------------+\n";

    intf.read((char*)this, sizeof(*this));
    int row = 0;

    while (!intf.eof())
    {
        row++;

        // Format the date with leading zeros
        stringstream dateStream;
        dateStream << setw(2) << setfill('0') << q << "/";
        dateStream << setw(2) << setfill('0') << B << "/";
        dateStream << setw(4) << setfill('0') << p;
        string formattedDate = dateStream.str();

        cout << "| " << setw(7) << left << row << " | " << setw(20) << left << auname << " | " << setw(16) << left << sc1 << " | " << setw(16) << left << bookname << " | " << setw(14) << left << sc << " | " << setw(6) << formattedDate << " |\n";
        cout << "---------------------------------------------------------------------------------------------------------\n";
        intf.read((char*)this, sizeof(*this));
    }
    intf.close();

    cout << "\n\t\t\t\t\t\tPress Any Key To Continue...";
    cin.ignore();
    cin.get();
}

else if (i == 3)
{
    system("cls");
    fflush(stdin);
    cout << "\n\t\t\t\t\t\t->Please Enter Details :-\n";
    cout << "\n\n\t\t\t\t\t\tEnter Student Name : ";
    cin.getline(st, 50);
    cout << "\n\n\t\t\t\t\t\tEnter Student's ID : ";
    cin.getline(st1, 20);
    system("cls");
    ifstream intf("student.txt", ios::binary);
    intf.read((char*)this, sizeof(*this));
    cont = 0;

    // Table header
    cout << "+-------------------------------------------------------------------------------------+\n";
    cout << "| Student Name     | Student ID   |  Book Name           | Book ID      |     Date    |\n";
    cout << "+-------------------------------------------------------------------------------------+\n";

    while (!intf.eof())
    {
        for (i = 0; sc1[i] != '\0' && st1[i] != '\0' && st1[i] == sc1[i]; i++);
        if (sc1[i] == '\0' && st1[i] == '\0')
        {
            cont++;

            if (cont == 1)
            {
                // Student details
                cout << "| " << setw(16) << left << auname << " | " << setw(12) << left << sc1;
            }

            // Book details
            cout << " | " << setw(20) << left << bookname << " | " << setw(12) << left << sc << " | " << setw(4) << right <<q << "/" << B << "/" << p << " |\n";
        }

        intf.read((char*)this, sizeof(*this));
    }
    intf.close();

    if (cont == 0)
        cout << "\n\t\t\t\t\t\tNo record found.";

    // Table footer
    cout << "+-------------------------------------------------------------------------------------+\n";
}

    else if(i==4)
    {
    system("cls");
    fflush(stdin);
    cout<<"\n\t\t\t\t\t\t->Please Enter Details :-\n";
    cout<<"\n\n\t\t\t\t\t\tEnter Student's ID : ";
    cin.getline(st,50);
    cout<<"\n\t\t\t\t\t\tEnter Book's ID : ";
    cin.getline(st1,20);
    fstream intf("student.txt",ios::in|ios::out|ios::ate|ios::binary);
    intf.seekg(0);
    intf.read((char*)this,sizeof(*this));
    while(!intf.eof())
        {
            for(i=0;sc[i]!='\0'&&st1[i]!='\0'&&st1[i]==sc[i];i++);
            for(j=0;sc1[j]!='\0'&&st[j]!='\0'&&st[j]==sc1[j];j++);
            if(sc[i]=='\0'&&sc1[j]=='\0'&&st[j]=='\0'&&st1[i]=='\0')
                {
                    d=q;
                    m=B;
                    y=p;
                    cout<<"\n\t\t\t\t\t\tEnter New Date : ";
                    cin>>q>>B>>p;
                    fine(d,m,y,q,B,p); //fn1
                    intf.seekp(intf.tellp()-sizeof(*this)); //fn3
                    intf.write((char*)this,sizeof(*this)); //fn5
                    cout<<"\n\n\t\t\t\t\t\tReissue Successfully."; //fn3
                    break;
                }
                   intf.read((char*)this,sizeof(*this));
        }
        intf.close();
    }
    else if(i==5)
    {
    system("cls");
    b=branch(2);
    system("cls");
    fflush(stdin);
    cout<<"\n\t\t\t\t\t\t->Please Enter Details :-\n";
    cout<<"\n\t\t\t\t\t\tEnter Book's ID : ";
    cin.getline(st1,20);
    der(st1,b,2);
    cout<<"\n\n\t\t\t\t\t\tEnter Student's ID : ";
    cin.getline(st,20);
    cout<<"\n\t\t\t\t\t\tEnter Present date : ";
    cin>>d>>m>>y;
    ofstream outf("temp.txt",ios::app|ios::binary);
    ifstream intf("student.txt",ios::binary);
    intf.read((char*)this,sizeof(*this));
    while(!intf.eof())
        {
            for(i=0;sc[i]!='\0'&&st1[i]!='\0'&&st1[i]==sc[i];i++);
            for(j=0;sc1[j]!='\0'&&st[j]!='\0'&&st[j]==sc1[j];j++);
            if(sc[i]=='\0'&&sc1[j]=='\0'&&st[j]=='\0'&&st1[i]=='\0'&&cont==0)
                {
                    cont++;
                    intf.read((char*)this,sizeof(*this));
                    fine(q,B,p,d,m,y);
                    cout<<"\n\t\t\t\t\t\tReturned Successfully.";
                }
            else
                {
                    outf.write((char*)this,sizeof(*this));
                    intf.read((char*)this,sizeof(*this));
                }
        }

    intf.close();
    outf.close();
    getch();
    remove("student.txt");
    rename("temp.txt","student.txt");
    }
    else if(i==6)
    {
    system("cls");
    librarian();
    }
    else
        cout<<"\n\t\t\t\t\t\tWrong Input.\n";

    cout<<"\n\n\t\t\t\t\t\tPress Any Key To Continue.....";
    getch();
    system("cls");
    librarian();
}
void Lib::fine(int d,int m,int y,int dd,int mm,int yy)
{
    long int n1,n2;
    int years,l,i;
    const int monthDays[12] = {31, 28, 31, 30, 31, 30,31, 31, 30, 31, 30, 31};
    n1 = y*365 + d;
    for (i=0; i<m - 1; i++)
        n1 += monthDays[i]; //fn1353
    years = y;
    if (m <= 2)
    years--;
    l= years / 4 - years / 100 + years / 400;
    n1 += l;
    n2 = yy*365 + dd;
    for (i=0; i<mm - 1; i++)
        n2 += monthDays[i];
    years = yy;
    if (m <= 2)
    years--;
    l= years / 4 - years / 100 + years / 400;
    n2 += l;
    n1=n2-n1;
    n2=n1-15;
    if(n2>0)
    cout<<"\n\t\t\t\t\t\tThe Total Fine is : "<<n2;

}
void Lib::der(char st[],int b,int x)
{
    int i,cont=0;
    fstream intf("Booksdata.txt",ios::in|ios::out|ios::ate|ios::binary);
    intf.seekg(0);
    intf.read((char*)this,sizeof(*this));
    while(!intf.eof())
    {
        for(i=0;b==B&&sc[i]!='\0'&&st[i]!='\0'&&st[i]==sc[i];i++);
        if(sc[i]=='\0'&&st[i]=='\0')
        {
            cont++;
            if(x==1)
            {
                q--;
            }
            else
            {
                q++;
            }
            intf.seekp(intf.tellp()-sizeof(*this));
            intf.write((char*)this,sizeof(*this));
            break;
        }
        intf.read((char*)this,sizeof(*this));
    }
    if(cont==0)
    {
        cout<<"\n\t\t\t\t\t\tBook Not Found.\n";
        cout<<"\n\n\t\t\t\t\t\tPress Any Key To Continue....!";
        getch();
        system("cls");
        issue();
    }
    intf.close();
}
void Lib::get()
 {
     int i;
        cout << "\t\t\t+----------------------------------------------------------------------+\n";
        cout << "\t\t\t|                       LIBRARY MANAGEMENT SYSTEM                      |\n";
        cout << "\t\t\t+----------------------------------------------------------------------+\n";
        cout << "\t\t\t+----------------------------------------------------------------------+\n";
        cout << "\t\t\t|                                                                      |\n";
        cout << "\t\t\t|                   Please Choose Any Option To login                  |\n";
        cout << "\t\t\t|                                                                      |\n";
        cout << "\t\t\t|1. Student                                                            |\n";
        cout << "\t\t\t|2. Librarian                                                          |\n";
        cout << "\t\t\t|3. Close Application                                                  |\n";
        cout << "\t\t\t|                                                                      |\n";
        cout << "\t\t\t+----------------------------------------------------------------------+\n";
        cout << "\t\t\t\t\t\t=>Enter Your Choice: ";
        cin >> i;

        if (i == 1) {
            system("cls");
            student();
        } else if (i == 2) {
            pass();
        } else if (i == 3) {
            exit(0);
        } else {
            cout << "\nPlease Enter Correct Option :(";
            getch();
            system("cls");
            get();
        }
    }

    void student() {
        // Student code goes here
    }

    void pass() {
        // Librarian code goes here
    }


void Lib::student()
{
     int i;
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout << "\t\t\t|                             WELCOME STUDENT                           |\n";
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout << "\t\t\t|                                                                       |\n";
        cout << "\t\t\t|                         Please Choose One Option:                     |\n";
        cout << "\t\t\t|                                                                       |\n";
        cout << "\t\t\t|1. View BookList                                                       |\n";
        cout << "\t\t\t|2. Search for a Book                                                   |\n";
        cout << "\t\t\t|3. Go To Main Menu                                                     |\n";
        cout << "\t\t\t|4. Close Application                                                   |\n";
        cout << "\t\t\t|                                                                       |\n";
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout << "\t\t\t\t\t\t=>Enter Your Choice: ";
        cin >> i;

        if (i == 1)
            booklist(1);
        else if (i == 2)
            see(1);
        else if (i == 3) {
            system("cls");
            get();
        } else if (i == 4)
            exit(0);
        else {
            cout << "\nPlease Enter Correct Option :(";
            getch();
            system("cls");
            student();
        }
    }

    void booklist(int userType) {
        // Booklist code goes here
    }

    void see(int userType) {
        // Search code goes here
    }

    void get() {
        // Main menu code goes here
    }
void Lib::pass()
{
    int i=0;
    char ch,st[21],ch1[21]={"1234"};
    cout<<"\n\t\t\t\t\t\tEnter Password : ";
    while(1)
    {
    ch=getch();
    if(ch==13)
    {
        st[i]='\0';
        break;
    }
    else if(ch==8&&i>0)
    {
        i--;
        cout<<"\b \b";
    }
    else
    {
    cout<<"*";
    st[i]=ch;
    i++;
    }
    }
    ifstream inf("password.txt");
    inf>>ch1;
    inf.close();
    for(i=0;st[i]==ch1[i]&&st[i]!='\0'&&ch1[i]!='\0';i++);
    if(st[i]=='\0'&&ch1[i]=='\0')
    {
        system("cls");
        librarian();
    }
    else
    {
        cout<<"\n\n\t\t\t\t\t\tWrong Password.\n\n\t\t\t\t\t\tTry Again.....\n";
        getch();
        system("cls");
        get();
    }
}
void Lib::librarian()
{
    int i;
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout << "\t\t\t|                           WELCOME LIBRARIAN                           |\n";
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout << "\t\t\t|                      >>Please Choose One Option:                      |\n";
        cout << "\t\t\t|1. View BookList                                                       |\n";
        cout << "\t\t\t|2. Search For a Book                                                   |\n";
        cout << "\t\t\t|3. Modify/Add Book                                                     |\n";
        cout << "\t\t\t|4. Issue Book                                                          |\n";
        cout << "\t\t\t|5. Go To Main Menu                                                     |\n";
        cout << "\t\t\t|6. Change Password                                                     |\n";
        cout << "\t\t\t|7. Close Application                                                   |\n";
        cout << "\t\t\t+-----------------------------------------------------------------------+\n";
        cout<<"\n\t\t\t\t\t\t=>Enter Your Choice : ";
        cin>>i;
        switch(i)
        {
            case 1:booklist(2);
                     break;
            case 2:see(2);
                     break;
            case 3:modify();
                     break;
            case 4:issue();
                     break;
            case 5:system("cls");
                     get();
                     break;
            case 6:password();
                    break;
            case 7:exit(0);
            default:cout<<"\n\t\t\t\t\t\tPlease Enter Correct Option :(";
            getch();
            system("cls");
            librarian();
        }
}
void Lib::password()
{
    int i=0,j=0;
    char ch,st[21],ch1[21]={"1234"};
    system("cls");
    cout<<"\n\n\t\t\t\t\t\tEnter Old Password : ";
    while(1)
    {
    ch=getch();
    if(ch==13)
    {
        st[i]='\0';
        break;
    }
    else if(ch==8&&i>0)
    {
        i--;
        cout<<"\b \b";
    }
    else
    {
    cout<<"*";
    st[i]=ch;
    i++;
    }
    }
    ifstream intf("password.txt");
    intf>>ch1;
    intf.close();
    for(i=0;st[i]==ch1[i]&&st[i]!='\0'&&ch1[i]!='\0';i++);
    if(st[i]=='\0'&&ch1[i]=='\0')
    {
        system("cls");
        cout<<"\n\t\t**The Password Should Be Less Than 20 Characters & Don't Use Spaces**\n\n";
        cout<<"\n\t\t\t\t\t\tEnter New Password : ";
        fflush(stdin);
        i=0;
        while(1)
        {
        j++;
        ch=getch();
        if(ch==13)
        {
            for(i=0;st[i]!=' '&&st[i]!='\0';i++);
            if(j>20 || st[i]==' ')
            {
                cout<<"\n\n\t\t\t\t\t\tYou Did't Follow The Instruction \n\n\t\t\t\t\t\tPress Any Key For Try Again.....";
                getch();
                system("cls");
                password();
                librarian();
            }
            st[i]='\0';
            break;
        }
        else if(ch==8&&i>0)
        {
            i--;
            cout<<"\b \b";
        }
        else
        {
        cout<<"*";
        st[i]=ch;
        i++;
        }
        }
        ofstream outf("password.txt");
        outf<<st;
        outf.close();
        cout<<"\n\n\t\t\t\t\t\tYour Password Has Been Changed Successfully.";
        cout<<"\n\t\t\t\t\t\tPress Any Key To Continue......";
        getch();
        system("cls");
        librarian();
    }
    else
    {
        cout<<"\n\n\t\t\t\t\t\tPassword Is Incorrect.....\n";
        cout<<"\n\t\t\t\t\t\tEnter 1 For Retry Or 2 For Menu";
        cin>>i;
        if(i==1)
        {
        system("cls");
        password();
        }
        else
        {
            system("cls");
            librarian();
        }
    }
}

int main()
{
    std::cout << "\t\t\t\t+------------------------------------------+\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|         LIBRARY MANAGEMENT PROJECT       |\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t+------------------------------------------+\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|      Developed By:                       |" << std::endl;
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|      Sanjida Akter Sompa(IT-20032)       |\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|      Iffat Jahan Nilima(IT-21063)        |\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|------------------------------------------|\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|      Submitted To:                       |" << std::endl;
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|      S.M. Shamim                         |" << std::endl;
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|      Assistant Professor                 |\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t|      ICT,MBSTU                           |\n";
    std::cout << "\t\t\t\t|                                          |\n";
    std::cout << "\t\t\t\t+------------------------------------------+\n";
    std::cout << "\t\t\t\t=> Press any key to enter the main program. \n";
    std::cout << "  ";
    _getch();
    system("cls");
    Lib obj;
    obj.get();
    getch();
    return 0;
}




